...
usage() {
    ...
    echo "  --clean         Clean build outputs before building (disables incremental builds)"
    echo ""
    echo "Examples:"
    ...
    echo "  $0 --clean --device    # Clean build for device"
}
...
if [ "$build_type" = "device" ]; then
    ...
    ./build-ios-arch.sh device arm64 $([ "$clean_build" = true ] && echo "--clean");

    cd $script_dir;

    # Move output to generic location
    if [ "$clean_build" = true ] && [ -d "src/skia/out/release-ios-device" ]; then
        rm -rf src/skia/out/release-ios-device;
    fi
    if [ -d "src/skia/out/release-ios-device-arm64" ]; then
    ...
    if [ "$build_universal" = true ]; then
        ...
        ./build-ios-arch.sh simulator x64 $([ "$clean_build" = true ] && echo "--clean");
        ./build-ios-arch.sh simulator arm64 $([ "$clean_build" = true ] && echo "--clean");

        cd $script_dir;

        # Create universal binary directory
        if [ "$clean_build" = true ] && [ -d "src/skia/out/release-ios-simulator" ]; then
        ...
        # Clean up individual architecture outputs (always clean temp files)
        ...
        ./build-ios-arch.sh simulator $current_arch $([ "$clean_build" = true ] && echo "--clean");
        ...
        if [ -d "src/skia/out/release-ios-simulator-$current_arch" ]; then
            cp -r src/skia/out/release-ios-simulator-$current_arch/* src/skia/out/release-ios-simulator/;
        fi

        # Clean up architecture-specific output (always clean temp files)
    ...
    if [ "$clean_build" = true ] && [ -d "src/skia/out/release-ios-simulator-universal" ]; then
    ...
    if [ "$clean_build" = true ] && [ -d "src/skia/out/xcframeworks" ]; then
    ...
    # Clean up intermediate directories (always clean temp files)
